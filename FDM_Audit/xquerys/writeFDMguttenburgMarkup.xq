...
let $doc := fn:doc("C:\GitHub\corpus-baudelaire\FDM_Audit\GuttenburgpoemsWithMarkup.xml")
...
<TEI xmlns="http://www.tei-c.org/ns/1.0">{$doc/tei:TEI/tei:teiHeader}<text><body>{$each}</body></text></TEI>

let $title := $each/tei:head/tei:title
let $path := string-join(("C:\GitHub\corpus-baudelaire\FDM_Audit\GuttenburgMarkup\", $title, ".xml"), "")
